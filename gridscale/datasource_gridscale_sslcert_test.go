package gridscale

import (
	"fmt"
	"testing"

	"github.com/hashicorp/terraform-plugin-sdk/helper/acctest"
	"github.com/hashicorp/terraform-plugin-sdk/helper/resource"
)

func TestAccdataSourceGridscaleSSLCert_basic(t *testing.T) {
	name := fmt.Sprintf("object-%s", acctest.RandString(10))
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		Providers:    testAccProviders,
		CheckDestroy: testAccCheckGridscaleSshkeyDestroyCheck,
		Steps: []resource.TestStep{
			{

				Config: testAccCheckDataSourceSSLCertConfig_basic(name),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "id"),
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "common_name"),
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "status"),
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "create_time"),
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "change_time"),
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "not_valid_after"),
					resource.TestCheckResourceAttr("data.gridscale_ssl_certificate.foo", "fingerprints.#", "1"),
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "fingerprints.0.md5"),
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "fingerprints.0.sha256"),
					resource.TestCheckResourceAttrSet("data.gridscale_ssl_certificate.foo", "fingerprints.0.sha1"),
				),
			},
		},
	})

}

func testAccCheckDataSourceSSLCertConfig_basic(name string) string {
	return fmt.Sprintf(`

resource "gridscale_ssl_certificate" "foo" {
  name   = "%s"
  private_key = "-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----"
  leaf_certificate = "-----BEGIN CERTIFICATE-----\\n87fnnaj9tE69\n-----END CERTIFICATE-----"
}

data "gridscale_ssl_certificate" "foo" {
	resource_id   = gridscale_ssl_certificate.foo.id
}`, name)
}
