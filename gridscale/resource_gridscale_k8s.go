...
func resourceGridscaleK8sRead(d *schema.ResourceData, meta interface{}) error {
	...
	if isNodePoolsSet {
		...
		for _, nodePoolSet := range nodePoolsSet {
			...
			if storageType, isStorageTypeSet := nodePoolSet["storage"]; isStorageTypeSet {
				nodePoolRead["storage_type"] = storageType
			}
			if rocketStorage, isRocketStorageSet := nodePoolSet["storage"]; isRocketStorageSet {
